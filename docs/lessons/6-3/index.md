...
6-2との違いは、フォームには**バリアント付きのButton**と、ラベル＋入力欄がひとまとまりになった**TextField**があることです。バリアントとはコンポーネントの見た目を切り替えるプロパティのことで、5-2で `variant: primary / secondary` を作りました。これがコード生成でどう扱われるかを、このレッスンで確認します。
