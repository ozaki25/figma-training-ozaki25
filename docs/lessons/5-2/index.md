...
1. TextFieldを作ったのと同じ、`task-form` フレームの外の空いた場所に、`t` でテキスト `タスクを登録`（14px、SemiBold、白）を作り、`Shift + A` で包む
   ...
   - 角丸8、背景 `brand`、幅は**内包**
2. `Cmd + Option + K` でコンポーネント化する
3. コンポーネントの名前を `Button`、中のテキストを `label` にする
4. コンポーネントを選択して、右サイドバー上部のひし形に矢印が刺さったアイコン（ホバーすると「バリアントの追加」と出ます）をクリックする。同じ見た目の2枚目が紫の破線枠のコンポーネントセットに入る
5. 右サイドバーの「現在のバリアント」で、`プロパティ1` を `variant` に変える
6. 1枚目の値を `primary`、2枚目を `secondary` にする。値を変えるときは、変えたいカード自身をキャンバスで選び直してから書き換える（1枚選んだままもう1枚の値は変えられない）
7. `secondary` の方を、背景 `surface`、枠線1px `brand`、テキスト色 `brand` に変える
