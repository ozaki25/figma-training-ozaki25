...
2. 入力ボックスを作る。四角形ではなくオートレイアウトフレームで作るのは、中に文字を入れられるようにするため
   1. テキスト `例: 会議資料の作成`（14px、`text/sub`）を置いて、`Shift + A` で横オートレイアウトにする
   2. 高さ44、パディング 12、角丸8、配置は左の中央にする
   3. 背景 `surface`、枠線1px `border` にする
   4. 名前を `input` にする。幅はまだ触らない（この時点では親がオートレイアウトではないので拡大を選べない）
...
6. 右サイドバーの「現在のバリアント」で、`プロパティ1` を `variant` に変える
7. 1枚目の値を `primary`、2枚目を `secondary` にする。値を変えるときは、変えたいカード自身をキャンバスで選び直してから書き換える（1枚選んだままもう1枚の値は変えられない）
8. `secondary` の方を、背景 `surface`、枠線1px `brand`、テキスト色 `brand` に変える
...
2. 3番目の `期限` と4番目の `優先度` のインスタンス2つを選択し、`Shift + A` で横オートレイアウトにまとめて名前を `field-row` にする
3. `field-row` を間隔 12、パディング 0、幅は拡大にする
4. 中の2つのTextFieldインスタンスは、それぞれ幅を拡大にする
5. 4番目の `優先度` のインスタンスの入力欄テキストを `選択する ▼` のように書き換えて、プルダウンらしく見せる。インスタンスの中身は、新しい要素の追加ではなくテキストの書き換えで対応する
