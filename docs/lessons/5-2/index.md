...
コンポーネント化は3-2と4-2、バリアントは3-2で練習したとおりです。迷ったらそちらに戻って確認してください。
...
2. 入力ボックスを作る。テキスト `例: 会議資料の作成`（14px、`text/sub`）を置いて `Shift + A` で横オートレイアウトにする。高さ44、パディング 12、角丸8、背景 `surface`、枠線1px `border`。名前を `input` にする。幅はまだ触らない（この時点では親がオートレイアウトではないので拡大を選べない）。入力ボックスを四角形ではなくオートレイアウトフレームで作るのは、中に文字を入れられるようにするため
3. `タスク名`（label）と `input` の2つを選択して `Shift + A` で縦オートレイアウト。間隔 8、パディング 0、幅は**固定320**にする
4. 中の `input` を選択し、幅を**拡大**にする。親のオートレイアウトができたので拡大が選べるようになっている。2-3でやった「外側から順に拡大」と同じ理屈で、こうしておくと後でインスタンスの幅を変えたとき入力欄も一緒に伸びる
5. `Cmd + Option + K`（Windowsは `Ctrl + Alt + K`）でコンポーネント化する
6. `Cmd + R` で、コンポーネントの名前を `TextField`、中の部品を `label` と `input` にする

### 2. Buttonを作って2バリアントにする

1. TextFieldを作ったのと同じ、`task-form` フレームの外の空いた場所に、テキスト `タスクを登録`（14px、SemiBold、白）を作り、`Shift + A` でオートレイアウトにする
2. 高さ44、左右パディング 20、配置は中央、角丸8、背景 `brand`、幅は**内包**に設定する
3. `Cmd + Option + K` でコンポーネント化する
4. コンポーネントの名前を `Button`、中のテキストを `label` にする
