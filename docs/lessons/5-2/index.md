...
2. 入力ボックスを作る。四角形ではなくオートレイアウトフレームで作るのは、中に文字を入れられるようにするため
   1. `t` でテキスト `例: 会議資料の作成`（14px、`text/sub`）を作り、`Shift + A` で包む
...
4. 間隔 8、パディング 0、幅は**固定320**にする
5. 中の `input` を選択し、幅を**拡大**にする。親のオートレイアウトができたので拡大が選べるようになっている。2-3でやった「外側から順に拡大」と同じ理屈で、こうしておくと後でインスタンスの幅を変えたとき入力欄も一緒に伸びる
6. `Cmd + Option + K`（Windowsは `Ctrl + Alt + K`）でコンポーネント化する
7. コンポーネントの名前を `TextField`、中の部品を `label` と `input` にする

### 2. Buttonを作って2バリアントにする

1. TextFieldを作ったのと同じ、`task-form` フレームの外の空いた場所に、`t` でテキスト `タスクを登録`（14px、SemiBold、白）を作り、`Shift + A` で包む
2. 次のとおり設定する
...
2. 3番目の `期限` と4番目の `優先度` のインスタンス2つを選択して `Shift + A` で横に包み、名前を `field-row` にする
