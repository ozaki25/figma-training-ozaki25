...
2. 入力ボックスを作る。四角形ではなくオートレイアウトフレームで作るのは、中に文字を入れられるようにするため
   1. `t` でテキスト `例: 会議資料の作成`（`body` + `text/sub`）を作り、`Shift + A` で包んで、名前を `input` にする
      - パディング `space/12`、配置は左中央
      - 角丸 `radius/8`、背景 `surface`、枠線1px `border`
      - 高さは**固定44**
   2. 幅はまだ触らない（この時点では親がオートレイアウトではないので拡大を選べない）
3. `label` と `input` の2つを選択して `Shift + A` で縦に包み、名前を `TextField` にする
...
2. `label` を選択して `Shift + A` で包み、名前を `Button` にする
