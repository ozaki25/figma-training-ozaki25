...
1. `task-form` フレームの外の空いた場所に、テキスト `タスク名`（12px、`text/sub`）を作る
2. 入力ボックスを作る: テキスト `例: 会議資料の作成`（14px、`text/sub`）を置いて `Shift + A` で横Auto Layoutにする。高さ44、padding 12、角丸8、背景 `surface`、枠線1px `border`。名前を `input` にし、幅を **Fill** にする。入力ボックスは四角形ではなくAuto Layoutフレームで作るので中に文字を入れられる。`input` を幅Fillにしておくと、後でインスタンスをFillにしたとき入力欄も一緒に横へ広がる
3. `タスク名`（label）と `input` の2つを選択して `Shift + A` で縦Auto Layout。gap 8、パディング 0
