# 6-4 タスク登録フォームをコード生成する

5-2ではタスク一覧をコードに変換しました。ここでは2画面目、この章で作った `task-form` を同じ流れでコードにします。
...
![Figma側のButtonコンポーネントセット（primaryとsecondaryの2バリアント）が、コード側では Button の variant プロパティに変換される対応関係の図](/lessons/6-4/variant-to-props.svg)
...
- src/App.tsx で、5-2で作った TaskList の下に並べて表示してください
```
...
1. Figmaで6章のファイルを開き、`task-form` フレームを選ぶ
2. 右クリック → **コピー/貼り付けオプション** → **選択範囲へのリンクをコピー** を選ぶ

### 2. Claude Codeに指示を投げる

上の雛形をコピーし、URLの行だけ、いまコピーした `task-form` のリンクに差し替えて送ります。
...
ブラウザは自動で更新されています。フォームカードの中にラベル付きの入力欄が並び（期限と優先度は横2列）、右下に「キャンセル」と紫の「タスクを登録」ボタンが出ていれば成功です。

### 5. TaskListとの整合性を見比べる

同じ画面に `TaskList` と `TaskForm` が縦に並んでいます。次の3点がそろっていれば、2画面の統一感がコードにも受け継がれています。
...
これでデザインからコードへの変換をひととおり体験しました。4章で作ったタスク一覧と、この章で作ったタスク登録フォームが、両方ともコードとして手元にあります。最後の7章で、2日間全体をふりかえります。
