...
5-2ではタスク一覧をコードに変換しました。ここでは2画面目、6章で作った `task-form` を同じ流れでコードにします。
...
これでデザインからコードへの変換を2画面ぶん体験しました。4章で作ったタスク一覧と6章で作ったタスク登録フォームが、両方ともコードとして手元にあります。最後の8章で、2日間全体をふりかえります。
