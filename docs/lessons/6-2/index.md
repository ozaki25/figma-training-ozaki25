...
このレッスンで実感してほしいのは、生成の質がFigma側の作り込みでほぼ決まるという点です。レイヤー名が良ければコンポーネント名やクラス名がそのまま良くなり、オートレイアウトが整っていればflexやgapの構造が正確に出ます。逆に整っていないと、その分だけ手戻りが増えます。3〜4章で命名やオートレイアウトを整えた効果が、ここで出てきます。
