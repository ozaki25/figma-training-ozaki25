...
このレッスンで実感してほしいのは、生成の質がFigma側の作り込みでほぼ決まるという点です。レイヤー名が良ければコンポーネント名やクラス名がそのまま良くなり、Auto Layoutが整っていればflexやgapの構造が正確に出ます。逆に整っていないと、その分だけ手戻りが増えます。3〜4章で命名やAuto Layoutを整えた効果が、ここで出てきます。
...
逆に、Figma側でレイヤー名が `Frame 42` のまま、Auto Layoutを使わずに座標で並べていると、生成されるコードは意味不明な要素名と絶対配置の羅列になります。手直しの手間は、そのまま元のデザインの整い具合で決まります。
