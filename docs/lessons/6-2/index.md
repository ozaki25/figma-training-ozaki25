...
このレッスンで実感してほしいのは、生成の質がFigma側の作り込みでほぼ決まるという点です。レイヤー名が良ければコンポーネント名やクラス名がそのまま良くなり、Auto Layoutが整っていればflexやgapの構造が正確に出ます。逆に整っていないと、その分だけ手戻りが増えます。3〜4章で命名やAuto Layoutにこだわった意味が、ここで一気に返ってきます。
...
真ん中の `get_design_context` は、Figma MCPが提供するツールです。フレームの構造・Auto Layoutの設定・スタイル・レイヤー名を一度にまとめて取得します。Claude Codeはこの中身を読み、指示に沿ってコードを組み立てます。ここで受け渡される情報の質は、Figma側のレイヤー名と構造の整い具合でほぼ決まります。
...
- **Tailwindクラス**: Auto Layoutの `gap` や `padding` が `gap-4` や `p-4` のような形で出ているか
- **色**: 4-3で決めた色（`brand`・`text/main` など）が近い値のTailwindクラス、または任意値の記法で反映されているか
...
2. `TaskCard` のprops（`title` と `due` を受け取る形になっているか）を見る
3. `TaskList` の中でダミーデータの配列を `map` して `TaskCard` を並べているかを見る

### 4. ローカルで表示してみる

生成された2ファイルを、既存のReactプロジェクト（Vite・Next.jsなど、手元にあるもの）の `src/components/task-list/` に置き、適当なページから `<TaskList />` を呼び出します。ブラウザで開いて、白いカードが縦に並び、左にチェックの丸、中央にタスク名、右端に期限が出れば成功です。
...
- **完了タスクの見せ方が反映されない**: 「4枚目のカードは完了タスクです。チェックの塗りを `bg-[#7C3AED]`、タイトルの色を薄いグレー、装飾に `line-through` を当ててください」
...
- **Auto Layoutが整っている**: gap・padding・整列が、Tailwindの `gap-*`・`p-*`・`items-*` に翻訳される
- **スタイルが登録されている**: `brand`・`text/main` のような名前で、色の意図が伝わる

逆に、Figma側でレイヤー名が `Frame 42` のまま、Auto Layoutを使わずに座標で並べていると、生成されるコードは意味不明な要素名と絶対配置の羅列になります。手直しの手間は、そのまま元のデザインの整い具合に跳ね返ります。

## ゴール確認

- [ ] `task-list` のリンクとスタック・分割・配置を含む指示をClaude Codeに投げられた
- [ ] 生成された `TaskList.tsx` と `TaskCard.tsx` の中身を読み、想定どおりの分割になっているか確認できた
- [ ] レイヤー名やAuto Layoutの整い具合が、生成コードの質に直結することを実感できた
