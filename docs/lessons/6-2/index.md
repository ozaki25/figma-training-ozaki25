...
| 例文テキスト | `期限` は `2026/07/10`、`優先度` は `選択する ▼`（プルダウンらしく見せる）、`カテゴリ` は `例: デザイン`。`タスク名` と `説明` は原本の `例: 会議資料の作成` のまま |
| `説明` の `input` | 高さを**固定96**に上書きした縦長 |
...
1. `説明` のインスタンスの中の `input` を選び、高さを**固定96**に上書きする。説明欄だけが縦長になり、例文は上端に残る
