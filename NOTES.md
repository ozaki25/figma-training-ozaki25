...
書き出す対象は色・余白・角丸の3種類。`text/main` は `--color-text-main`、`space/16` は `--spacing-16`、`radius/8` は `--radius-8` と、Figmaのバリアブル名をそのまま使う。クラスは `bg-brand`・`p-16`・`rounded-8` になる。`get_variable_defs` はテキストスタイルも返すので、プロンプト側で除外している。

`--spacing-16` を定義するとTailwindの `p-16` が64pxから16pxに変わり、`p-4` は16pxから4pxになる（実測済み）。デザインルールで定義した値が正なので、標準の目盛りが置き換わるのは意図どおり。Tailwindのデフォルトを基準に書き出す範囲を決めると、4章で余白と角丸のトークンを作らせた意味がコードに出なくなる。

`get_variable_defs` はフレームが使っているものだけを返す。`task-list` は26個（色15・余白7・角丸4）で、`space/48` と `radius/16` は含まない。この2つは `task-form` が使うので、7-1に足りないぶんを追加する手順を置いた。値は小文字の16進数で返る。
