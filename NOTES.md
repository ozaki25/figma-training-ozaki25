...
- TaskCardは8枚→5枚に削減（2026-07-27決定）。カードの書き換えが旧4-4遅延の主因だったため。今日3枚（4-3の流用）+ 今週1枚 + あとで1枚で、4-5で新しく書き換えるのは2枚だけ。セクション3つの構成は維持
- 5枚化に伴い、教材Figmaファイルの修正とキャプチャの撮り直しが必要（`4-4/task-list-final.png`、`6-3/two-screens-side-by-side.png`）。撮影はユーザーが行う
