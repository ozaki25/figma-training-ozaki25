...
- スタイルの連続性（2026-07決定）: 練習と本番は同一ファイルのためスタイルを共有する。3-1で `surface`・`text/main`・`text/sub` を登録（全員のカードに実在する色のみ。brandは登録しない）。4-3では surface を使い回し、text/main・text/sub は定義を本番の値に更新、brand・bg・border・bg-active とテキスト5つを新規登録する。二重登録と値の衝突を避けるための設計
...
- Variables: 2026年時点で色・数値などを保存できる別機能。ライト/ダークのモード切替やデザイントークンに使う。単一値のみでグラデーション等は保存できない。公式も「Variablesはスタイルの置き換えではない」としている
  - 当初は「名前の紹介にとどめる」判断だったが、デザイントークン管理は必須テーマというユーザー判断により**レッスン3-4として新設（2026-07決定の変更）**。無料プランではモード追加が使えない（1モードのみ）ため、3-4はプリミティブ→セマンティックの2層と伝播の体験に絞り、モードは紹介のみ。4章以降の本番はスタイル運用のまま変更しない
...
- 5-3は新しい機能を教えるレッスンではなく「2画面を並べて見比べ、ズレを直す」実践レッスン。
  観点は4つ（ヘッダー・色・タイポ・余白/角丸）に絞り、6章のコード生成に向けた動機付けを短く添える
