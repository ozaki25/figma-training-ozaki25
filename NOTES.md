...
## 章立ての組み替え: コード生成を2画面目より前へ（2026-07-27決定）

リハーサル（午後のみ・受講者4名）で4-4（旧・仕上げレッスン）が46〜123分かかり、当日はコード生成まで到達しなかった。「デザインだけやって終わる」のを避けるため、画面を1つ作るたびにコードへ変換する並びに組み替えた。
...
| 5-1 / 5-2 / 5-3（フォーム） | 6-1 / 6-2 / 6-3 |
| 6-1 / 6-2（MCPセットアップ・一覧のコード生成） | 5-1 / 5-2 |
| 6-3 フォームのコード生成 | 6-4 |

- 章の性格を混ぜないため、レッスンを他章の中へ移すのではなく章ごと入れ替えた。5章は「Figma MCP + Claude Codeでコード化」のまま残るので、目次からコード生成の章が消えることはない
- 4-4の分割の切れ目は「部品を作る（StatCard・filter-bar・NavItem）」と「画面を仕上げる（ロゴ・ユーザーブロック・セクション分け・done）」。分割しても総量は変わらないので、時間の圧縮は別途検討する（カード8枚→5枚案が未決）
- URLが変わるレッスンがある。旧URLからのリダイレクトは用意していない（社内限定・noindexのため許容）
