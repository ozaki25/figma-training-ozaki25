...
## 章立ての組み替え: 8章立てにして、コード生成を2画面目より前へ（2026-07-27決定）

リハーサル（午後のみ・受講者4名）で旧4-4が46〜123分かかり、コード生成まで到達しなかった。「デザインだけやって終わる」のを避けるため、Day2を「画面を作る章 → その画面をコードにする章」の2周にする。章は入れ替えではなく、4章と6章（旧5章）の後ろにコード化の章をそれぞれ挿す。ユーザー決定は3点。8章立てを基本方針とする / MCPセットアップは前にくくり出さず5章の1本目のまま / 4-4は長すぎるので分割し、2度目のコード生成は1度目の参照で済ませる。

旧→新の対応:

| 旧 | 新 |
| --- | --- |
| 4-4 一覧画面を仕上げて完成させる | 4-4 統計・フィルタ・ナビの部品を作る / 4-5 一覧画面を仕上げて完成させる（分割） |
| 5-1 / 5-2 / 5-3（フォーム） | 6-1 / 6-2 / 6-3（6章） |
| 6-1 / 6-2（MCPセットアップ・一覧のコード生成） | 5-1 / 5-2（5章） |
| 6-3 フォームのコード生成 | 7-1（7章、1レッスンの章） |
| 7-1 ふりかえり | 8-1（8章） |

- 全8章・21レッスン。URLが変わるレッスンがある。旧URLからのリダイレクトは用意していない（社内限定・noindexのため許容）
- 7-1は5-2の手順を繰り返さず、参照と差分（バリアント→props）だけを扱う
- TaskCardは8枚→5枚に削減（2026-07-27決定）。カードの書き換えが旧4-4遅延の主因だったため。4-5で新しく書き換えるのは2枚だけになる

## 8章（まとめ）を廃止（2026-07-27決定）

8-1は唯一演習のないレッスンで、CLAUDE.mdの「読むだけのレッスンにしない」に反していた。俯瞰図と各章のまとめはトップのカリキュラム表と重複し、残るのは自己チェック3項目だけだった。章ごと削除し、全7章・20レッスンに戻した。

- 「2日間で通ったルート」の俯瞰図はトップの「カリキュラム」節の冒頭へ移し、`docs/public/two-day-overview.svg` に置いた
- 「身についたことの確認」は捨てた
- 研修の締めはレッスンではなく口頭で行う。タイムテーブルの17:10枠に置いた

## TextFieldにmultilineバリアントを追加（2026-07-28決定）

6-2の「インスタンスの中の `input` の高さを固定96に上書きする」は、Figmaの仕様上できない操作だった。インスタンスの中の要素はサイズを変えられない（3-2で教えているとおり）。教材ファイルの見本も、説明欄だけインスタンスを切り離してしのいでいた。

`TextField` に `multiline=false/true` を追加し、`true` 側の原本で `input` を高さ96にする形に変更した。説明欄はトグルを切り替えるだけになり、「状態はバリアントで表現し、上書きで作らない」という設計v4の方針とも一致する。

あわせて説明欄の例文を `例: 共有する資料の要点をまとめる` に変更した。タスク名と同じ `例: 会議資料の作成` で、説明欄のプレースホルダーとして意味が通っていなかった。

## 講師ノートを廃止（2026-07-28決定）

`docs-internal/lecture-notes.md` を削除した。レッスン順に「話すこと」「見るところ」を並べたものだったが、研修中に読みながら進められる粒度にならず、ユーザーが使わない判断をした。Notion側の写しも今後更新しない。

リハーサルで実際に出たつまずきポイントと、押したときに削る箇所は `lecture-schedule.md` に移した。

## task-appを2ページ構成にする（2026-07-28決定）

7-1の指示テンプレートが「5-2で作った TaskList の下に並べて表示」で、一覧の下にスクロールするとフォームが出る画面になっていた。7-1の演習5「2画面を見比べる」のためと説明していたが、一覧880px・フォーム746pxで1画面に収まらず、理由が成立していなかった。演習5の中身も色や角丸の値の確認なので、同時に見える必要はない。
...
書き出す対象は色だけにした。余白と角丸はTailwindの標準の目盛りと完全に一致することを実測で確認している（`--spacing: 0.25rem` なので `space/16` は `p-4`、`--radius-lg: 0.5rem` なので `radius/8` は `rounded-lg`。8個の余白トークンと5個の角丸トークンすべてに標準クラスの対応がある）。`get_variable_defs` はテキストスタイルも返すので、プロンプト側で除外している。

`task-list` フレームに `get_variable_defs` をかけると15個のセマンティック色トークンが全部返ることも確認済み。本文に載せた `@theme` の全文と受講者の手元の出力が一致する。値は小文字の16進数で返る。

書き出す対象を色だけにした理由は、4章のトークンの性質の違いで説明する。`brand`・`text/main` は意味の名前なのでコード側にも名前で渡す価値があるが、`space/16`・`radius/8` は値の名前で、Tailwindの `p-4`・`rounded-lg` がすでに同じ役目を果たしている。「数値が一致するから」という説明では、4章で余白と角丸のトークンを作らせた意味を否定してしまう。

技術的には余白は書き出してはいけない。`--spacing-4: 4px` を定義するとTailwindの `p-4` が16pxから4pxに変わり、標準クラスの意味が静かに壊れる（実測済み）。角丸は `--radius-8` が `rounded-8` という別のクラスになるだけで衝突しないので書き出せるが、値の名前を渡す意味がないので書き出さない。
